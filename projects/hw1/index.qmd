...
pct_ci_low, pct_ci_high = np.quantile(boot_thetas, [0.025, 0.975])
...
bootstrap_tbl = pl.DataFrame({
    "Quantity": [
        ...
        "95% CI lower bound (normal approx.)",
        "95% CI upper bound (normal approx.)",
        "95% CI lower bound (percentile)",
        "95% CI upper bound (percentile)"
    ],
    "Value": [
        theta_hat,
        boot_se,
...
Using the bootstrap results, I report both a Normal-approximation interval and a percentile bootstrap interval. In this simulation, both intervals tell the same story: the plausible values of $\theta$ remain above zero. That suggests CTA A likely outperforms CTA B in this run, not just at the point estimate but across a reasonable range of uncertainty.
...
The p-value is the probability, under the null hypothesis, of observing a test statistic at least as extreme as the one in this sample.
...
The simulated false positive rate under repeated peeking is much higher than the nominal 5% rate from a single test. That means if I keep checking the results and stop as soon as I see significance, I increase the overall Type I error rate and become much more likely to falsely declare a winner when the two CTAs are actually identical.
...
Most importantly, this assignment shows that A/B testing is not just a business tool; it is also a statistical problem. Randomized experiments produce noisy data, and frequentist methods give us a principled way to estimate treatment effects, quantify uncertainty, and avoid being fooled by random variation. In a marketing setting, that matters because decisions about rollout, messaging, and resource allocation should depend not only on which version looks better in one sample, but also on how confident we are that the improvement is real.
